...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const deskIconBorderRadiusTop = attributes[`${ICON_BORDER_RADIUS_TOP}DeskRange`];
	const tabIconBorderRadiusTop = attributes[`${ICON_BORDER_RADIUS_TOP}TabRange`];
	const mobIconBorderRadiusTop = attributes[`${ICON_BORDER_RADIUS_TOP}MobRange`];
	const iconBorderRadiusTopUnit = attributes[`${ICON_BORDER_RADIUS_TOP}Unit`];
	// Icon Border Radius Right
	const deskIconBorderRadiusRight = attributes[`${ICON_BORDER_RADIUS_RIGHT}DeskRange`];
	const tabIconBorderRadiusRight = attributes[`${ICON_BORDER_RADIUS_RIGHT}TabRange`];
	const mobIconBorderRadiusRight = attributes[`${ICON_BORDER_RADIUS_RIGHT}MobRange`];
	const iconBorderRadiusRightUnit = attributes[`${ICON_BORDER_RADIUS_RIGHT}Unit`];
	// Icon Border Radius Bottom
	const deskIconBorderRadiusBottom = attributes[`${ICON_BORDER_RADIUS_BOTTOM}DeskRange`];
	const tabIconBorderRadiusBottom = attributes[`${ICON_BORDER_RADIUS_BOTTOM}TabRange`];
	const mobIconBorderRadiusBottom = attributes[`${ICON_BORDER_RADIUS_BOTTOM}MobRange`];
	const iconBorderRadiusBottomUnit = attributes[`${ICON_BORDER_RADIUS_BOTTOM}Unit`];
	// Icon Border Radius Left
	const deskIconBorderRadiusLeft = attributes[`${ICON_BORDER_RADIUS_LEFT}DeskRange`];
	const tabIconBorderRadiusLeft = attributes[`${ICON_BORDER_RADIUS_LEFT}TabRange`];
	const mobIconBorderRadiusLeft = attributes[`${ICON_BORDER_RADIUS_LEFT}MobRange`];
	const iconBorderRadiusLeftUnit = attributes[`${ICON_BORDER_RADIUS_LEFT}Unit`];
	// Image Size
						...
						<div key={index} className="bwdssb-service-item bwdssb-bgColor">
							{item.showLink ? (
								<a href={item.boxLink} target={item.newTab ? '_blank' : ''} rel="noreferrer noopener">
									<div className="content-wrap">
										<div className="service-icon-media">
											...
											{item.chooseMedia === 'svg' && item.svgContent && (
												<div>
										...
										<RichText
											...
											value={item.title}
											onChange={(newText) => updateItemText(index, 'title', newText)}
											placeholder={__('Write title……', 'bwdssb-service-showcase')}
										/>
										<RichText
											tagName="p"
											className="service-description"
											value={item.description}
											onChange={(newText) => updateItemText(index, 'description', newText)}
											placeholder={__('Write description……', 'bwdssb-service-showcase')}
										/>
									</div>
								</a>
							) : (
								<div className="content-wrap">
									<div className="service-icon-media">
										{item.chooseMedia === 'icon' &&  (
											<div className="service-icon">{icons[item.icon || 'gear']}</div>
										)}
										{item.chooseMedia === 'image' && item.imageUrl && (
											<img src={item.imageUrl.url} alt={item.text1} />
										)}
